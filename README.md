...
| UI framework | React + TypeScript | Component-based architecture that integrates cleanly with a Three.js render loop (via refs/hooks) and keeps input/visualization/export modules decoupled (RNF-06). TypeScript is required, not optional: `packages/schema` only prevents contract drift between `client` and `server` if both consumers are statically typed against it. |
| API layer | Node.js + Express + TypeScript | Thin orchestration layer: input validation, session persistence, REST contract with the client. Deliberately does not own numerical computation. TypeScript for the same reason as the client: shared contract enforcement via `packages/schema`. |
